(function () {
  const STATS_URL = '/api/social-stats';
  ...
  async function fetchStats() {
    try {
      ...
      console.warn('Primary fetch to /api/social-stats failed, trying absolute fallback...', err);
      try {
        const fallbackRes = await fetch(FALLBACK_URL);
        if (!fallbackRes.ok) throw new Error('HTTP status ' + fallbackRes.status);
        return await fallbackRes.json();
      } catch (err2) {
        console.warn('Fallback fetch failed, using local mock data.', err2);
        return fallbackData;
      }
    }
  }
  ...
  async function init() {
    ...
    if (statsGrid) {
      statsGrid.innerHTML = `
        <div class="stat-card">
          <div class="stat-card__icon-wrap" style="color: #ef4444; background: rgba(239, 68, 68, 0.08);">
            ${HEART_SVG}
          </div>
        ...
        <div class="stat-card">
          <div class="stat-card__icon-wrap" style="color: #3b82f6; background: rgba(59, 130, 246, 0.08);">
            ${EYE_SVG}
          </div>
          <div class="stat-card__body">
