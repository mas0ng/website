...
The bio page remains noindex and is excluded from sitemap-static.xml. Snapshot refresh updates its public profile metadata when the profile content changes. Main public pages share one Person identifier (https://mas0ng.com/#person); the dedicated ProfilePage is /bio.html#profile. All public HTML pages link to /llms.txt with rel="describedby" for compatible agents. This discovery hint does not guarantee Google rankings. Keep schema factual and consistent with published content.

### Automated SEO consistency

After public snapshots refresh, refresh-seo.mjs updates only head metadata: shared Person profile links, certification ItemList markup and legal WebPage descriptions. Credential metadata uses only published names, issuers and public verification links; it adds no ratings, accreditation or employment claims. The generator refuses body changes and keeps modification dates stable on unchanged builds. Publications use build-publications.mjs and write their own sitemap at /publications/sitemap.xml; the root sitemap is an index for the core and publications sitemaps. check-seo.mjs validates sitemap coverage, unique titles/descriptions, canonical and sharing URLs, JSON-LD, certificate counts, public indexing and error-page exclusions before publishing. Run both scripts after any manual metadata change.
...
Image SEO: the bio primaryImageOfPage and its image sitemap entry reference the original avatar actually displayed there. Snippet exclusions are limited to copyright text, the temporary privacy notice and report-form UI, using supported span/div containers; public identity and certification content remains eligible.
